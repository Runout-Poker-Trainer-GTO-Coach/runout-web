...
/**
 * Reports without an explicit `status` are treated as Active.
 * @param {unknown} v
...
/**
 * One-line chapter identifier: "Chapter 1 — What Is a Value Bet?"
...
function lessonLabel(row) {
  ...
  if (id && title) return `Chapter ${id} — ${title}`
  if (id) return `Chapter ${id}`
  if (title) return title
  return '(no chapter)'
}

/**
 * "Slide 1 of 7 · content" — slideIndex is 0-based in the source data.
 * @param {Record<string, unknown>} row
 */
function slideLabel(row) {
  const idxRaw = row.slideIndex
  const idx =
    typeof idxRaw === 'number' && Number.isFinite(idxRaw)
      ? idxRaw
      : Number.parseInt(asStr(idxRaw), 10)
  const totalRaw = row.totalSlides
  const total =
    typeof totalRaw === 'number' && Number.isFinite(totalRaw)
      ? totalRaw
      : Number.parseInt(asStr(totalRaw), 10)
  const kind = asStr(row.slideKind).trim()

  const human = Number.isFinite(idx) ? idx + 1 : null
  let core = ''
  if (human != null && Number.isFinite(total)) core = `Slide ${human} of ${total}`
  else if (human != null) core = `Slide ${human}`
  else if (Number.isFinite(total)) core = `${total} slides`
  return kind ? (core ? `${core} · ${kind}` : kind) : core || '—'
}

/**
 * Stable grouping key for "the same lesson" — `skillId|lessonId`.
 * Falls back to title-based identity if both ids are missing.
 * @param {Record<string, unknown>} row
 */
function lessonGroupKey(row) {
  const skill = asStr(row.skillId).trim().toLowerCase()
  const id = asStr(row.lessonId).trim()
  if (skill || id) return `s:${skill}|l:${id}`
  return `t:${asStr(row.moduleTitle).trim().toLowerCase()}|${asStr(row.lessonTitle).trim().toLowerCase()}`
}

/**
 * Stable grouping key for "the same slide within a lesson".
 * @param {Record<string, unknown>} row
 */
function slideGroupKey(row) {
  return `${lessonGroupKey(row)}|i:${asStr(row.slideIndex).trim()}`
}
...
export default function LessonReportsPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const snap = await getDocs(collection(db, lessonReportsCollectionName))
        if (cancelled) return
        ...
        if (!cancelled) {
          setError(e?.message || 'Failed to load lesson reports')
          setRows([])
        }
      } finally {
  ...
  /**
   * Group every report by chapter + slide. Reports inside a group are
   * sorted newest-first.
   *
  ...
  /** Apply status filter + search, then sort groups by latest submission. */
  const displayGroups = useMemo(() => {
    ...
    return allGroups
      ...
      .map((g) => g) // shallow copy not needed; sort below copies
      .sort((a, b) => {
        const aMs = Math.max(
          0,
  ...
  const resolveReport = useCallback(async (firestoreDocId) => {
    if (!db) return
    ...
    try {
      await updateDoc(doc(db, lessonReportsCollectionName, firestoreDocId), {
        status: RESOLVED,
  ...
  const resolveGroup = useCallback(
    async (group) => {
      ...
      try {
        await Promise.all(
          activeIds.map((id) =>
            updateDoc(doc(db, lessonReportsCollectionName, id), {
              status: RESOLVED,
              resolvedAt: serverTimestamp(),
            }),
          ),
  ...
  const saveNotes = useCallback(
    async (firestoreDocId, notesText) => {
      ...
      try {
        await updateDoc(
          doc(db, lessonReportsCollectionName, firestoreDocId),
          {
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
          ...
          <div>
            ...
            <p className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-600">
              {rows.length > 0 ? (
                ...
                <span>Collection: {lessonReportsCollectionName}</span>
              ) : null}
            </p>
          </div>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-stretch">
          <div className="relative w-full min-w-[12rem] sm:w-auto">
            <Filter
              ...
              aria-hidden
            />
            <label className="sr-only" htmlFor="lesson-reports-status-filter">
              Filter by status
            </label>
            <select
              id="lesson-reports-status-filter"
              value={statusFilter}
              onChange={(e) =>
                setStatusFilter(
              ...
              className="w-full cursor-pointer appearance-none rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-3 text-sm text-slate-900 shadow-sm outline-none transition hover:border-slate-300 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/15 sm:min-w-[12rem]"
            >
          ...
          <div className="relative w-full min-w-[220px] sm:min-w-[22rem] sm:max-w-xl sm:flex-1 lg:max-w-2xl">
            ...
            />
            <input
              id="lesson-reports-search"
              type="search"
              placeholder="Search module, lesson, user, details, issue…"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-3 text-sm text-slate-900 shadow-sm outline-none placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/15"
            />
          </div>
        </div>
      </header>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-lg shadow-slate-900/[0.04] ring-1 ring-slate-900/[0.02]">
        {loading ? (
          <div className="flex flex-col items-center justify-center gap-4 py-24 text-slate-600">
            <Loader2
              className="size-9 animate-spin text-indigo-600"
              strokeWidth={2}
              aria-hidden
            />
            <p className="text-sm font-medium">Loading lesson reports…</p>
          </div>
        ) : rows.length === 0 ? (
          ...
          <div className="flex flex-col items-center px-4 py-20 text-center">
            ...
            <p className="mt-1 max-w-xs text-xs text-slate-500">
              Try another status, search, or clear the search field.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[880px] text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/90">
                  ...
                  <th
                    ...
                    title="Total reports for this chapter + slide (all-time, includes resolved)."
                  >
                    <span className="inline-flex items-center gap-1.5">
                      <Flag
              ...
              <tbody className="divide-y divide-slate-100">
                {displayGroups.map((group) => {
                  ...
                  const module = asStr(latest.moduleTitle).trim()
                  const skill = asStr(latest.skillId).trim()
                  const lesson = lessonLabel(latest)
                  const slide = slideLabel(latest)
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-3 lg:px-4">
                        <div className="space-y-1.5">
                          <div className="flex flex-wrap items-center gap-1.5">
                            <span className="inline-flex items-center gap-1 rounded-md bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-900 ring-1 ring-indigo-100">
                              <Layers
                                className="size-3 shrink-0"
                                strokeWidth={2.25}
                                aria-hidden
                            ...
                            {skill && skill.toLowerCase() !== module.toLowerCase() ? (
                              <span
                          ...
                          <p className="text-xs text-slate-500" title={slide}>
                            {slide}
                          </p>
                        </div>
                      </td>
                        ...
                        {distinctAppVersions.length === 1 ? (
                          ...
                          <p
                            ...
                            title={distinctAppVersions
                              ...
                              .join(', ')}
                          >
                            v{asStr(latest.appVersion).trim() || distinctAppVersions[0]}{' '}
                      ...
                      <td
                        ...
                        title={`${totalCount} all-time report${totalCount === 1 ? '' : 's'} for this chapter + slide (includes resolved)`}
                      >
                        <span className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 px-3 py-1 ring-1 ring-indigo-100">
                          <Flag
                            className="size-3.5 shrink-0 text-indigo-600"
                            strokeWidth={2.25}
                            aria-hidden
                      ...
                      <td className="px-3 py-3 text-center lg:px-4">
                        <div className="inline-flex flex-col items-stretch gap-1.5">
                          <button
                            ...
                            className="inline-flex cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 text-xs font-semibold text-indigo-900 transition hover:border-indigo-300 hover:bg-indigo-100/80 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"
                          >
                            <Eye
                            ...
                            View
                            {totalCount > 1 ? (
                              <span className="rounded-full bg-indigo-200/80 px-1.5 py-0.5 font-mono text-[10px] text-indigo-900">
                                {totalCount}
                              </span>
                            ) : null}
                          </button>
      ...
      {detailGroup ? (
        <ReportGroupModal
          ...
          onSaveNote={(docId) =>
            saveNotes(docId, noteDrafts[docId] ?? '')
          }
...
function ReportGroupModal({
  ...
  const moduleTitle = asStr(head.moduleTitle).trim()
  const skill = asStr(head.skillId).trim()
  const lesson = lessonLabel(head)
  const slide = slideLabel(head)
  const totalCount = group.reports.length
  const activeCount = group.reports.filter(
    (r) => normalizeStatus(r.status) === ACTIVE,
  ).length
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="lesson-report-modal-title"
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-md bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-900 ring-1 ring-indigo-100">
                <Layers
                  ...
                  aria-hidden
                />
                {moduleTitle || '(no module)'}
              </span>
              {skill && skill.toLowerCase() !== moduleTitle.toLowerCase() ? (
                <span
                  className="rounded-md bg-slate-100 px-1.5 py-0.5 font-mono text-[10.5px] text-slate-600"
                  title="skillId"
                >
            ...
            <p className="mt-1 text-sm font-medium text-slate-800">{lesson}</p>
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 cursor-pointer rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
            aria-label="Close"
          >
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-slate-100 px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
          <button
            type="button"
            onClick={onClose}
            className="cursor-pointer rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
          >
            Close
          </button>
          {activeCount > 0 ? (
...
function ReportItemCard({
  ...
  const inputId = `lesson-report-notes-${report.firestoreDocId}`
    ...
    <li className="list-none rounded-xl border border-slate-200 bg-slate-50/70 p-3 sm:p-4">
      ...
      <div className="mt-2.5">
        ...
        <textarea
          ...
          className="mt-1.5 w-full resize-y rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm outline-none placeholder:text-slate-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 disabled:cursor-not-allowed disabled:opacity-60"
        />
        <div className="mt-1.5 flex flex-wrap items-center justify-between gap-2">
          ...
          <button
            ...
            className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1 text-[11px] font-semibold text-indigo-900 transition hover:border-indigo-300 hover:bg-indigo-100/80 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {saving ? (
              <>
                <Loader2
